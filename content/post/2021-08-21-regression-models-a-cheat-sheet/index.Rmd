...
categories: ["methods"]
tags: ["regression", "brms", "lmer", "gamm"]
math: true
---

# One model to rule them all

**Regression models**, aka linear regression models or linear models, are a group of statistical models based on the simple idea that we can predict an outcome variable $Y$ based on a function $f(X)$.

The "simplest" regression model is the formula of a line:^[Technically, the "simplest" regression model is $y = f(x)$, but oh well...]
...
**This means that if you master the principles of regression models, then you can virtually fit any kind of data using regressions.**
You can bid farewell to ANOVAs, $t$-tests, $\chi^2$-tests, and what not.
In fact, these can all be thought of as specific cases of regressions.
...
Understanding that these named models are in fact all regression models gives you super powers you can use on data (Sauron will be so jealous):
...
You can fit a regression model with smooth terms with brms (simply add smooth terms to the formula) or with mgcv (using `gam()` or `bam()`), among others.
